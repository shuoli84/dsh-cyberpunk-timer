...
.dsh-cyber-clock__next { position: relative; width: 224px; margin-top: 4px; color: rgba(184,234,242,0.78); font-size: 9px; line-height: 1.3; letter-spacing: 0.1em; text-transform: uppercase; font-variant-numeric: tabular-nums; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
.dsh-cyber-clock__next-label { color: #49eeff; }
.dsh-cyber-clock__next-time { color: #ff63df; }
...
`;
    ...
    function computeNextEvent(now, alarms) {
      const nowMs = now.getTime();
      let next = null;
      for (const alarm of alarms) {
        let target = alarmTimeMs(now, alarm.hhmm, 0);
        if (target === null) continue;
        if (target <= nowMs) target = alarmTimeMs(now, alarm.hhmm, 1);
        if (target === null) continue;
        if (next === null || target < next.target) next = { alarm, target };
      }
      return next;
    }
    ...
    function apply(ctx) {
      ...
      function Clock() {
        ...
        const dragSession = React.useRef(null);
        ...
        const nextEvent = React.useMemo(
          () => computeNextEvent(now, state.alarms),
          [now, state.alarms]
        );
        const nextCountdown = nextEvent === null ? null : formatDuration(nextEvent.target - now.getTime());
        ...
        function beginDrag(event) {
          ...
          dragSession.current = {
            ...
            startDx: drag.dx,
            startDy: drag.dy,
            baseLeft: rect.left - drag.dx,
            baseTop: rect.top - drag.dy
          };
          event.currentTarget.setPointerCapture(event.pointerId);
          setDrag({ active: true, dx: drag.dx, dy: drag.dy });
        }
        ...
        function moveDrag(event) {
          ...
          const rect = event.currentTarget.getBoundingClientRect();
          const maxDx = window.innerWidth - rect.width - session.baseLeft;
          const maxDy = window.innerHeight - rect.height - session.baseTop;
          const nextDx = clamp(session.startDx + event.clientX - session.startX, -session.baseLeft, maxDx);
          const nextDy = clamp(session.startDy + event.clientY - session.startY, -session.baseTop, maxDy);
          setDrag({ active: true, dx: nextDx, dy: nextDy });
        }
        ...
        const clockElement = h(
          ...
          nextEvent === null ? null : h("div", { className: "dsh-cyber-clock__next", "aria-live": "polite" },
            h("span", { className: "dsh-cyber-clock__next-label" }, "NEXT "),
            nextEvent.alarm.label,
            " · ",
            h("span", { className: "dsh-cyber-clock__next-time" }, nextCountdown)
          ),
